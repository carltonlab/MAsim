...
def main() -> None:
    ...
    mutation_counts = np.zeros((4 * num_pairs, chromosome_size), dtype=np.float64)
    ...
    for _ in range(iterations):
        ...
        chromosomes = np.vstack((male_y, male_x, female_x2, female_x3))
        mutation_counts += (chromosomes > 3).astype(np.float64)
    ...
    if final_male_y is not None:
        ...
        np.savetxt("mutation_frequency.txt", mutation_counts / iterations, fmt="%.6f")
